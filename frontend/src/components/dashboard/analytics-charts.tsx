...
import { PieChart, BarChart3, MapPin, Sparkles, Flame, Zap, Snowflake } from 'lucide-react';
...
export const AnalyticsCharts = ({ leads }: AnalyticsChartsProps) => {
  ...
  // 3. Calculate ICP Grade Distribution (HOT / WARM / COLD)
  let hotCount = 0;
  let warmCount = 0;
  let coldCount = 0;

  leads.forEach((l) => {
    const grade = (l.lead_grade || 'COLD').toUpperCase();
  ...
  return (
    <div className="space-y-5 font-sans">
      ...
      {/* ICP Lead Quality Breakdown Widget (HOT / WARM / COLD) */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
              ICP Quality Grade Distribution
            </h4>
            <p className="text-[11px] text-slate-500">Automated AI Lead Readiness Rating</p>
          </div>
        </div>

        <div className="grid grid-cols-3 gap-3 w-full md:w-auto">
          {/* HOT */}
          <div className="bg-emerald-50 border border-emerald-200/80 px-4 py-2 rounded-xl text-center min-w-[100px]">
            <div className="flex items-center justify-center gap-1 text-emerald-700 font-extrabold text-xs">
              <Flame size={12} /> HOT ({hotPct}%)
            </div>
            <div className="text-lg font-black text-emerald-900 mt-0.5">{hotCount}</div>
          </div>

          {/* WARM */}
          <div className="bg-amber-50 border border-amber-200/80 px-4 py-2 rounded-xl text-center min-w-[100px]">
            <div className="flex items-center justify-center gap-1 text-amber-700 font-extrabold text-xs">
              <Zap size={12} /> WARM ({warmPct}%)
            </div>
            <div className="text-lg font-black text-amber-900 mt-0.5">{warmCount}</div>
          </div>

          {/* COLD */}
          <div className="bg-slate-100 border border-slate-200 px-4 py-2 rounded-xl text-center min-w-[100px]">
            <div className="flex items-center justify-center gap-1 text-slate-600 font-extrabold text-xs">
              <Snowflake size={12} /> COLD ({coldPct}%)
            </div>
            <div className="text-lg font-black text-slate-800 mt-0.5">{coldCount}</div>
          </div>
        </div>
      </div>
    </div>
